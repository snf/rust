...
extern "rust-intrinsic" {
    ...
    /// Equivalent to the appropriate `llvm.memcpy.element.unordered.atomic.p0i8.p0i8.*` intrinsic,
    /// with a size of `count` * `size_of::<T>()`, an alignment of `min_align_of::<T>()`, and an
    /// element size of `size_of::<T>()`.
    ///
